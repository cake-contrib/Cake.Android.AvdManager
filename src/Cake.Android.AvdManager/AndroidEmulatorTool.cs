...
namespace Cake.AndroidEmulator
{
	internal partial class AndroidEmulatorTool : Tool<AndroidEmulatorToolSettings>
	{
		...
		internal IEnumerable<string> Run(ICakeContext context, string avdName, AndroidEmulatorToolSettings settings, params string[] args)
		{
			if (settings == null)
				settings = new AndroidEmulatorToolSettings();
			...
			return run(context, settings, builder);
		}
